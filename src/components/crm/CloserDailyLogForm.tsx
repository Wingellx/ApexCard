    ...
    return (
      <label className="flex items-center gap-3 cursor-pointer group">
        <input
          ...
          className="w-4 h-4 rounded border-[#1e2130] bg-[#0d0f15] accent-indigo-500"
        />
  ...
  if (field.field_type === "duration") {
    return (
      <div className="relative">
        ...
        />
        <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1 text-[11px] text-[#4b5563]">
...
export default function CloserDailyLogForm({ fields, todayValues, onOpenBuilder }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Inject hidden field_ids list and log_date
    fd.set("field_ids", activeFields.map(f => f.id).join(","));
    fd.set("log_date", today);

    // Boolean checkboxes not checked won't be in FormData — explicitly set false
  ...
  // Split booleans out — they get their own row at the bottom
  const numberFields  = activeFields.filter(f => f.field_type !== "boolean");
