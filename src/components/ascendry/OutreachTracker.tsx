...
import { Plus, Trash2, Pencil, X, Check, TrendingUp } from "lucide-react";
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, Cell } from "recharts";
import { Card, CardHeader, CardBody, CardTitle } from "@/components/ui/Card";
import Button from "@/components/ui/Button";
import { addProspect, updateProspect, deleteProspect } from "@/app/ascendry/actions";
...
export default function OutreachTracker({ prospects: initial }: { prospects: Prospect[] }) {
  ...
  // Template performance chart data
  const templateChartData = useMemo(() => {
    const byTemplate: Record<string, { sent: number; replied: number; positive: number }> = {};
    for (const t of TEMPLATES) byTemplate[t] = { sent: 0, replied: 0, positive: 0 };
    ...
    for (const p of prospects) {
      ...
      if (p.reply_status === "Positive" || p.reply_status === "Call Booked") {
        byTemplate[p.template_used].positive++;
      }
    }

    return TEMPLATES.map(t => ({
      name: `Template ${t}`,
      replyRate: byTemplate[t].sent > 0
        ? Math.round((byTemplate[t].replied / byTemplate[t].sent) * 100)
        : 0,
  ...
  const bestTemplate = useMemo(() => {
    const best = [...templateChartData].sort((a, b) => b.replyRate - a.replyRate)[0];
    return best?.sent > 0 ? best.name : null;
  }, [templateChartData]);
  ...
  // Optimistic helpers
  function optimisticAdd(p: Prospect) {
  ...
  async function handleAdd() {
    ...
    startTransition(async () => {
      try {
        ...
        // revalidation will re-sync from server
      }
    });
  }
  ...
  async function handleSaveEdit(id: string) {
    ...
    startTransition(async () => {
      try {
        await updateProspect(id, editState);
      } catch {}
    });
  }
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Weekly Template Performance</CardTitle>
          {bestTemplate && (
        ...
        <CardBody>
            ...
            <BarChart data={templateChartData} barSize={36}>
              ...
              <Bar dataKey="replyRate" radius={[4, 4, 0, 0]}>
                {templateChartData.map((entry) => (
                  <Cell
                    key={entry.name}
                    fill={entry.name === bestTemplate ? "#7c3aed" : "#3f3f46"}
                  />
                ))}
              </Bar>
            </BarChart>
          ...
          <div className="flex gap-4 mt-2">
            {templateChartData.map(t => (
              <div key={t.name} className="text-xs text-zinc-500">
                <span className="text-zinc-400 font-medium">{t.name}</span>: {t.sent} sent
              </div>
            ))}
          </div>
        </CardBody>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <Field label="Date Messaged">
                <input
                  type="date"
                  className="input-base"
                  value={form.date_messaged}
                  onChange={e => setForm(f => ({ ...f, date_messaged: e.target.value }))}
                />
              </Field>
              <Field label="Template">
                <select
                  ...
                  onChange={e => setForm(f => ({ ...f, template_used: e.target.value }))}
                >
                  {TEMPLATES.map(t => <option key={t} value={t}>Template {t}</option>)}
                </select>
              </Field>
                ...
                <select
                  ...
                  onChange={e => {
                    ...
                    setForm(f => ({
                      ...f,
                      reply_status: rs,
                      call_booked: rs === "Call Booked" ? true : f.call_booked,
                    }));
                  }}
                >
              ...
              <Field label="Call Booked">
                <div className="flex items-center gap-3 h-9">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={form.call_booked}
                      onChange={e => setForm(f => ({ ...f, call_booked: e.target.checked }))}
                  ...
                  <tr key={p.id} className="border-b border-[#1e2130] hover:bg-zinc-800/20 transition-colors">
                    <td className="px-4 py-3 font-medium text-zinc-100 whitespace-nowrap">
                      {isEditing ? (
                        <input
                          ...
                          value={editState.prospect_name ?? ""}
                          onChange={e => setEditState(s => ({ ...s, prospect_name: e.target.value }))}
                        />
                      ) : p.prospect_name}
                    </td>
                    <td className="px-4 py-3 text-zinc-400">
                      {isEditing ? (
                        <input
                          ...
                          value={editState.instagram_handle ?? ""}
                          onChange={e => setEditState(s => ({ ...s, instagram_handle: e.target.value || null }))}
                        />
                      ) : (p.instagram_handle ?? "—")}
                    </td>
                    <td className="px-4 py-3 text-zinc-400 whitespace-nowrap">
                      {isEditing ? (
                        <input
                          ...
                          value={editState.date_messaged ?? ""}
                          onChange={e => setEditState(s => ({ ...s, date_messaged: e.target.value }))}
                        />
                      ) : p.date_messaged}
                    </td>
                    <td className="px-4 py-3">
                      {isEditing ? (
                        <select
                          className="input-base w-28"
                          value={editState.template_used ?? ""}
                          onChange={e => setEditState(s => ({ ...s, template_used: e.target.value as Prospect["template_used"] }))}
                        >
                          {TEMPLATES.map(t => <option key={t} value={t}>Template {t}</option>)}
                        </select>
                      ) : (
                    ...
                    <td className="px-4 py-3">
                      {isEditing ? (
                        <select
                          className="input-base w-36"
                          value={editState.reply_status ?? "No Reply"}
                          onChange={e => setEditState(s => ({ ...s, reply_status: e.target.value as Prospect["reply_status"] }))}
                        >
                          {REPLY_STATUSES.map(s => <option key={s} value={s}>{s}</option>)}
                        </select>
                      ) : (
                    ...
                    <td className="px-4 py-3">
                      {isEditing ? (
                        <input
                          type="checkbox"
                          checked={editState.call_booked ?? false}
                          onChange={e => setEditState(s => ({ ...s, call_booked: e.target.checked }))}
                    ...
                    <td className="px-4 py-3 text-zinc-500 max-w-[160px] truncate">
                      {isEditing ? (
                        <input
                          ...
                          onChange={e => setEditState(s => ({ ...s, notes: e.target.value || null }))}
                        />
                      ) : (p.notes ?? "—")}
                    </td>
                    <td className="px-4 py-3">
